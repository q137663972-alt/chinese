 ...
 *   语文出题 = 汉字/拼音/笔画   → 英语出题 = 本年级词表（听音/看图/中英互译）
 *
 * 【加载链路（为什么只加这一个文件就够）】
 ...
 * ==================================================================== */
(function () {
  try { if (typeof log === "function") log("en battle file executing"); } catch (e) {}
  if (typeof registerGame !== "function") {
    try { if (typeof log === "function") log("en battle: registerGame missing"); } catch (e) {}
  ...
  /* 兜底词表：数据层万一没装配好（内容包没拉到、单元结构变了、某些机顶盒上
     window.DATA 是空的），宁可用这一小撮一年级常用词开局，也绝不能让界面出现
     undefined 或空题。手机端一直正常、机顶盒 undefined 就是这么兜住的。 */
  var FALLBACK = [
    { e: "book", z: "书", k: "📖" }, { e: "ruler", z: "尺子", k: "📏" },
    { e: "pencil", z: "铅笔", k: "✏️" }, { e: "schoolbag", z: "书包", k: "🎒" },
    { e: "teacher", z: "老师", k: "👩🏫" }, { e: "eraser", z: "橡皮", k: "🧽" },
    { e: "face", z: "脸", k: "😊" }, { e: "eye", z: "眼睛", k: "👁️" },
    { e: "ear", z: "耳朵", k: "👂" }, { e: "nose", z: "鼻子", k: "👃" },
    { e: "mouth", z: "嘴巴", k: "👄" }, { e: "hand", z: "手", k: "✋" },
    { e: "dog", z: "狗", k: "🐶" }, { e: "cat", z: "猫", k: "🐱" },
    { e: "bird", z: "鸟", k: "🐦" }, { e: "tiger", z: "老虎", k: "🐯" },
    { e: "monkey", z: "猴子", k: "🐵" }, { e: "rabbit", z: "兔子", k: "🐰" },
    { e: "apple", z: "苹果", k: "🍎" }, { e: "banana", z: "香蕉", k: "🍌" },
    { e: "milk", z: "牛奶", k: "🥛" }, { e: "water", z: "水", k: "💧" },
    { e: "red", z: "红色", k: "🔴" }, { e: "blue", z: "蓝色", k: "🔵" },
    { e: "green", z: "绿色", k: "🟢" }, { e: "yellow", z: "黄色", k: "🟡" },
    { e: "one", z: "一", k: "1️⃣" }, { e: "two", z: "二", k: "2️⃣" },
    { e: "three", z: "三", k: "3️⃣" }, { e: "four", z: "四", k: "4️⃣" }
  ];

  var WORDS = [];            /* 本年级词表（当前单元的词排在最前） */
  var ALLWORDS = [];         /* 全年级词表，同单元凑不够干扰项时向这里借 */
  ...
  function buildBank() {
    ...
    /* 当前单元的词排在最前：知识圈首先是「复习本单元」，再拿本年级其它单元掺着考 */
    try {
      if (g && g.books && g.books[bi] && g.books[bi].u && g.books[bi].u[ui]) {
        var ws = g.books[bi].u[ui].w || [];
        for (var i = 0; i < ws.length; i++) if (ws[i] && ws[i].e && ws[i].z) WORDS.push(ws[i]);
      }
    } catch (e) {}
    try {
      if (g && g.books) for (var b = 0; b < g.books.length; b++) {
        ...
        for (var u = 0; u < us.length; u++) {
          var w = (us[u] && us[u].w) || [];
          for (var k = 0; k < w.length; k++) if (w[k] && w[k].e && w[k].z) ALLWORDS.push(w[k]);
        }
      }
    } catch (e) {}
    if (WORDS.length < 4) WORDS = ALLWORDS.slice();
    if (WORDS.length < 4) { WORDS = FALLBACK.slice(); ALLWORDS = FALLBACK.slice(); }
  }

  /* 出干扰项：先在本单元里挑，不够再向本年级借；永远排除正确答案本身 */
  function distractor(field, val, n) {
    var out = [], seen = {};
    if (val != null) seen[String(val)] = 1;
    var pool = shuffle(WORDS).concat(shuffle(ALLWORDS));
    var guard = 0;
  ...
  /* 出一道英语题：4 种题型轮换（听音选英文 / 看图选英文 / 看中文选英文 / 看英文选中文） */
  function makeQuestion() {
    var w = pick(WORDS) || pick(ALLWORDS) || FALLBACK[0];
    var type = pick(["listen", "pic", "cn2en", "en2cn"]);
    var en = T(w.e), cn = T(w.z), icon = T(w.k) || "🔤";
    if (!en) return { qText: "暂无题目", qBig: "", speakText: "", hint: "", opts: [], correct: 0 };
    var opts = [], correct = 0, qText = "", qBig = "", speakText = "", hint = "";
    if (type === "listen") {
      qText = "🔊 听一听，选出听到的单词"; qBig = "🔊"; speakText = en; hint = cn;
      opts = shuffle([en].concat(distractor("e", en, 3))).map(function (x) { return { label: T(x), val: T(x) }; });
      correct = idx(opts, function (o) { return o.val === en; });
    } else if (type === "pic") {
      qText = "看图片，选出这个单词"; qBig = icon; speakText = en; hint = cn;
      opts = shuffle([en].concat(distractor("e", en, 3))).map(function (x) { return { label: T(x), val: T(x) }; });
      correct = idx(opts, function (o) { return o.val === en; });
    } else if (type === "cn2en") {
      qText = "「" + cn + "」的英文是？"; qBig = cn; speakText = en; hint = en.charAt(0).toUpperCase();
      opts = shuffle([en].concat(distractor("e", en, 3))).map(function (x) { return { label: T(x), val: T(x) }; });
      correct = idx(opts, function (o) { return o.val === en; });
    } else { /* en2cn */
      qText = "「" + en + "」是什么意思？"; qBig = en; speakText = en; hint = "中文意思";
      opts = shuffle([cn].concat(distractor("z", cn, 3))).map(function (x) { return { label: T(x), val: T(x) }; });
      correct = idx(opts, function (o) { return o.val === cn; });
    }
    if (correct < 0) correct = 0;
    if (!opts.length) opts = [{ label: en, val: en }];
    return { qText: qText, qBig: qBig, speakText: speakText, hint: hint, opts: opts, correct: correct };
  }
  ...
  function ensureStyle() {
    ...
    s.textContent =
      ...
      ".b-q .qbig{font-size:38px;font-weight:900;line-height:1.2;word-break:break-word}" +
      ".b-q .qtext{font-size:16px;font-weight:800;margin:4px 0;color:#333}" +
      ...
      ".b-opt{display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(180deg,#fff,#f3f7ff);border:2px solid #e3e9f5;border-radius:16px;padding:14px 10px;font-size:19px;font-weight:900;color:#234;cursor:pointer;min-height:64px}" +
  ...
  function showQuestion() {
    ...
    if (S.buff) { /* 应用上一题攒出的知识发射器效果（一次性） */
      if (S.buff === "time") { S.time = Math.min(22, S.time + 3); S.left = S.time; banner("🔤 拼读泡泡枪！本题 +3 秒"); }
      if (S.buff === "hint") { S.showHint = true; banner("📖 单词卡发射器！显示中文提示"); }
      S.buff = null;
    }
    battleRender();
  ...
  function fireLauncher() {
    ...
    S.buff = kind;
    banner(kind === "time" ? "🔤 拼读泡泡枪已上膛！" : "📖 单词卡发射器已上膛！");
  }
  ...
  function bestKey() { return "el_battle_best"; }
  ...
  function startBattle() {
    buildBank();
    if (!WORDS.length) { if (typeof toast === "function") toast("词库为空"); return; }
  ...
  try { if (typeof log === "function") log("en battle registering"); } catch (e) {}
  registerGame({
    id: "battle",
    name: "知识圈",
    icon: "🪂",
  ...
  try { if (typeof log === "function") log("en battle registered, GAMES=" + ((window.GAMES && window.GAMES.length) || 0)); } catch (e) {}
})();
