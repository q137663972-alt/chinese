 ...
 *
 * ----------------------------------------------------------------------------
 * 2.5.0「通用空壳」与 2.4.2 兼容（2026-09-21 明确）
 * ----------------------------------------------------------------------------
 * · 2.5.0 壳（本文件所在的 APK）是「通用空壳」：boot.js 的 SUBJS 登记了
 *   语文/数学/英语三科，学科页由 js/subject.js 动态列出，APK 本身不锁定任何一科。
 *   「内容全部走热更」——tools/gen-pack.mjs 把三科的 js/css/图片/音频整包打出，
 *   设备启动 3 秒后后台装上，下次打开即生效，无需出新 APK。
 * · 2.4.2 是「语文单科壳」：它的内置 boot.js 只认语文。兼容靠两层：
 *   1) 资源包门槛 min_apk 恒为 3（见下方 DEFAULT_MIN_APK 注释），绝不跟着
 *      新 APK 的 versionCode 自动抬高 —— 否则 2.4.2(apk=11) 会被整份拒收。
 *   2) legacy/ 目录里的一批根级 js 桩（js/app.js 看门人 + js/bridge.js 桥接层 +
 *      game-battle.js 等空壳）随包下发，让老 boot.js 也能渲染选学科页并加载三科。
 *   ★ 改包时若真要「低于某版本必须换壳」，只能显式 --min-apk N 并在提交里写清原因。
 *
